pro close_euvsa_file
common write_to_euvsa_file_cal, euvsa_lun, last_hr, lastyd_h, format
...
default_file_name = getenv('exis_data_quicklook')+'/latest_euvsa_hour.txt'

this_hr = long(euvsa[0].time[0].sod[0]*24./86400.)
yd = (euvsa.time.yd)[0]
...
if thisyd_h ne lastyd_h then begin
   ...
   if size(euvsa_lun,/type) ne 0 then begin
      ; old file needs to be closed
      close_euvsa_file

      ; use year/doy hierarchy
      dest_path = getenv('exis_data_l0b_euvsa') + '/' + $
                  strmid(lastyd_h,0,4) + '/' + $
                  strmid(lastyd_h,4,3) + '/'
                  ;string(yd/1000,form='(i4.4)') + '/' + $
                  ;string(yd mod 1000,form='(i3.3)') + '/'
      ; if dir doesn't exist, create it and all parents
      if file_test(dest_path,/dir) eq 0 then file_mkdir, dest_path
      n=0L
      strn = string(n,form='(i3.3)')
      dest = dest_path + 'euvsa_' + lastyd_h + '_'+ strn +'.txt'
      ...
      print,' WRITE_TO_EUVSA_FILE: moving '+default_file_name+' to '+dest
      file_move, default_file_name, dest
   endif ; otherwise there is no previous file to close
   ; now open the file
   openw, euvsa_lun, default_file_name, /get_lun
   printf, euvsa_lun, ';record={time_yd:0L, time_hms:"", time_sod:0.0, user_flags:0UL, '+$
...
;   printf,euvsa_lun,';yyyydoy-hms sod user_flags power inttime test cal data[24] offset[24] runCtrl dataMode pwrCtrlStat integCnt cal_cycles cal_vMin cal_vMax cal_vStepUp cal_tStepUp cal_vStepDown cal_tStepDown door_pos filter_pos tempa tempb vernum type sechdrflag apid seqflag pktseqcnt pktlen day ms microsec'
   ...
   printf,euvsa_lun,';format='+strcompress(format)
   printf,euvsa_lun,';end_of_header'

endif

userf=euvsa.sec_hdr.uf
cal = euvsa.asic.cal
fsw = euvsa.fsw
mech= euvsa.exis_mech
pri=euvsa.pri_hdr
sec=euvsa.sec_hdr
ptime=euvsa.time

; write the data record
printf, euvsa_lun, ptime.yd, ptime.hms, ptime.sod, sec.userflags, $
        userf.TimeValid, userf.FSWBootRam, userf.ExisPowerAB, userf.ExisMode, userf.FM, userf.configID, $
        euvsa.diodes, $
        euvsa.offset, $
        euvsa.asic.runCtrl, euvsa.asic.SciMode, euvsa.asic.pwrStatus, euvsa.asic.integTime, $
        euvsa.ff.power, euvsa.ff.level, euvsa.ifBoardTemp, euvsa.fpgaTemp, euvsa.pwrSupplyTemp, euvsa.caseHtrTemp, euvsa.aTemp, euvsa.bTemp,euvsa.slitTemp, $
        fsw.invalidFlags, fsw.detChg_cnt, fsw.ffChg_cnt, $
;euvsa.asic.cal.asicCalRamp, euvsa.asic.cal.asicCalCyclesRemaining, $
