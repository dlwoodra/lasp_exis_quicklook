...
; 07/19/2017  Don Woodraska  Added Netcdf4 compatiblity, added unsigned data types and 64-bit type
;                            There only seems to be unsigned 64-bit type (ULONG64), no signed 64-bit type (LONG64)
;
; $Log: read_netcdf.pro,v $
; Revision 10.1  2017/06/13 23:53:25  dlwoodra
; added code to remove bad characters from netcdf variable names
;
;idver='$Id: read_netcdf.pro,v 10.1 2017/06/13 23:53:25 dlwoodra Exp $'
;
;+
...
for k=0, finq.nvars-1 do begin
	...
	case strupcase(var_def.datatype) of
	  ...
	  'UBYTE': tyeType = 1 ; treat the same as BYTE
		...
		'UINT': theType = 12
		'ULONG': theType=13
		;'INT64':theType=14 ; not supported?
		'ULONG64':theType=15
		'UINT64':theType=15
		else: theType = 5		; default is DOUBLE
	endcase
	;
    ...
    ; need to remove bad characters to ensure variable names are valid
    ; remove ~!@#$%^&*()_-+=/.,<>?"';:][{}`
    ; version > 8.4 has an undocumented replace method for IDL variables
    if float(!version.release) ge 8.4 then begin
       thebadchars = '~!@#$%^&*()_-+=/.,<>?";:][{}`|\' + "'" ; note that single quote is special
       n_badchars = strlen(thebadchars)
       for ibad = 0, n_badchars-1 do tmp_name = tmp_name.replace(strmid(thebadchars,ibad,1), '_')
    endif
	...
	case ndim_array of
		...
		1:  begin
			case var_inq[k].type of 
				...
				12: theData = uintarr( dim_size[ var_inq[k].dim[0] ] )
				13: theData = ulonarr( dim_size[ var_inq[k].dim[0] ] )
				14: theData = lon64arr( dim_size[ var_inq[k].dim[0] ] )
				15: theData = ulon64arr( dim_size[ var_inq[k].dim[0] ] )
		...
		2:  begin
			case var_inq[k].type of 
				...
				12: theData = uintarr( dim_size[ var_inq[k].dim[0] ], dim_size[ var_inq[k].dim[1] ] )
				13: theData = ulonarr( dim_size[ var_inq[k].dim[0] ], dim_size[ var_inq[k].dim[1] ] )
				14: theData = lon64arr( dim_size[ var_inq[k].dim[0] ], dim_size[ var_inq[k].dim[1] ] )
				15: theData = ulon64arr( dim_size[ var_inq[k].dim[0] ], dim_size[ var_inq[k].dim[1] ] )
